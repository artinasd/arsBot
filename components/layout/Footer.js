...
export default function Footer() {
  return (
    <footer id="contact" className="border-t border-white/8 bg-[#060709]">
      <div className="nova-container py-16 sm:py-20">
        <div className="grid gap-12 md:grid-cols-[1.5fr_1fr_1fr]">
          <div>
            <div className="mb-5 flex items-center gap-3">
              <span className="grid size-9 place-items-center rounded-xl bg-[#c8ff3d] text-[#08090b] text-lg font-extrabold">ن</span>
          ...
          <div>
            ...
            <div className="flex flex-col gap-3">
              {footerLinks.map((link) => (
                <a key={link.href} href={link.href} className="w-fit text-sm text-white/55 transition hover:text-white">
                  {link.label}
                </a>
              ))}
            </div>
        ...
        <div className="mt-16 flex flex-col gap-4 border-t border-white/7 pt-6 text-[11px] text-white/25 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex gap-5">
            <a href="#privacy" className="transition hover:text-white/60">حریم خصوصی</a>
            <a href="#terms" className="transition hover:text-white/60">شرایط استفاده</a>
          </div>
        </div>
      </div>
    </footer>
