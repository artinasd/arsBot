  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50 border-b border-white/5 bg-[#08090b]/75 backdrop-blur-2xl">
      <div className="nova-container flex h-[76px] items-center justify-between">
        <a href="#top" className="group flex items-center gap-3" aria-label="نُوا، صفحه اصلی">
        ...
        <div className="hidden items-center gap-3 md:flex">
          <a href="#contact" className="rounded-full border border-white/10 px-5 py-2.5 text-[12px] font-semibold text-white/75 transition hover:border-white/25 hover:text-white">
            ارتباط با ما
          </a>
          <a href="#products" className="group flex items-center gap-2 rounded-full bg-[#c8ff3d] px-5 py-2.5 text-[12px] font-bold text-[#08090b] transition hover:bg-[#d5ff6a]">
            محصولات
            <ArrowIcon />
          </a>
        </div>
      ...
      <div className={`overflow-hidden border-t border-white/5 transition-all duration-300 md:hidden ${open ? "max-h-96" : "max-h-0"}`}>
        <nav className="nova-container flex flex-col gap-1 py-4" aria-label="ناوبری موبایل">
          ...
          <a href="#contact" onClick={() => setOpen(false)} className="mt-2 rounded-2xl bg-[#c8ff3d] px-4 py-3 text-center text-sm font-bold text-[#08090b]">
            ارتباط با ما
          </a>
        </nav>
      </div>
