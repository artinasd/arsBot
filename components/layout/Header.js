    ...
    <header className="fixed inset-x-0 top-0 z-50 border-b border-slate-200/80 bg-white/90 backdrop-blur-2xl">
      <div className="nova-container flex h-[76px] items-center justify-between">
        ...
        <div className="hidden items-center gap-3 md:flex">
          <Link href="/contact" className="rounded-full border border-slate-200 bg-white px-5 py-2.5 text-[12px] font-semibold !text-slate-700 transition hover:border-slate-300 hover:bg-slate-50 hover:!text-slate-950">ارتباط با ما</Link>
          <Link href="/products" className="rounded-full bg-slate-950 px-5 py-2.5 text-[12px] font-bold !text-white transition hover:bg-[#3158e8] hover:!text-white">مشاهده محصولات</Link>
        </div>
      ...
      <div className={`overflow-hidden border-t border-slate-200 bg-white/95 transition-all duration-300 md:hidden ${open ? "max-h-96" : "max-h-0"}`}>
        <nav className="nova-container flex flex-col gap-1 py-4" aria-label="ناوبری موبایل">
          ...
          <Link href="/contact" onClick={() => setOpen(false)} className="mt-2 rounded-2xl bg-slate-950 px-4 py-3 text-center text-sm font-bold !text-white transition hover:bg-[#3158e8] hover:!text-white">ارتباط با ما</Link>
        </nav>
      </div>
    </header>
  );
